...
public class Level2Ending : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        cameraPrivate = GetComponent<Level2Ending>().cam;
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hitInfo, distance, mask))
        {
            ...
            if (Input.GetKeyDown(KeyCode.E) && interacting == true)
            {
                ...
                if (hitInfo.collider.CompareTag("Airlock Terminal"))
                {
                ...
                if (hitInfo.collider.CompareTag("Airlock Swipe Access") && dialogueOver == false)
                {
                    var terminalDialogue = GetComponent<Level2Dialogue>().airlockSwipe;
                    var terminalTrigger = FindObjectOfType<DialogueTrigger>();
                    terminalTrigger.dialogue = terminalDialogue;
                    terminalTrigger.TriggerDialogue();
                    dialogueOver = true;
                }
                else if (hitInfo.collider.CompareTag("Airlock Swipe Access") && dialogueOver == true)
                {
